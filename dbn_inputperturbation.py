...
def launch(config, print_fn):
    ...
    def step_label(state, batch):
        ...
        for i, res_params_dict in enumerate(resnet_param_list):
            if batch.get("images_tar") is not None:
        ...
        images = batch["images"]
        zB, logitsB = forward_resnet(resnet_param_list[0], images)
        logitsB = logitsB - logitsB.mean(-1, keepdims=True)
    ...
    def step_mixup(state, batch):
        ...
        mixed_tar = jnp.where(lamda < 0.5, x, perm_x)
        mixed_x = jnp.where(count == batch_size, mixed_x, x)
        mixed_y = jnp.where(count == batch_size, mixed_y, y)
        mixed_tar = jnp.where(count == batch_size, mixed_tar, x)

        batch["images"] = mixed_x
        batch["images_tar"] = mixed_tar
