...
def launch(config, print_fn):
    ...
    if config.diffcls:
        ...
        init_z = jnp.empty((1, *ctx_dim), model_dtype)
        variables = score_func.init(
            {"params": init_rng},
            rng=init_rng,
            x0=init_x,
            z1=init_z,
            cls_params=variables_clsB["params"] if not config.checker else variables_clsA["params"],
            training=False
        )
    elif config.ldm:
        init_x = jnp.empty((1, *x_dim), model_dtype)
    ...
    def step_train(state, batch, config):
        ...
        def loss_fn(params):
            ...
            if config.diffcls:
                ...
                zB = contexts
                if config.train_cls:
                    assert config.checker == False
                    cls_params = bparams
                elif config.checker:
                    cls_params = variables_clsA["params"]
                else:
                    cls_params = variables_clsB["params"]
                z0 = f_normalize(batch["references"]
                                 ) if config.checker else None
                (
            ...
            if config.diffcls:
                ...
                if config.mse_recon > 0:
                    ...
                    logp = jax.nn.log_softmax(new_logitsA/T, axis=-1)
                    q = jnp.exp(logq)
                    kld = jnp.sum(q*(logq-logp), axis=-1)
                    auloss0 = kld
                else:
                    auloss0 = recon
                ...
                references = f_normalize(batch["references"])
                reference_prior = config.z_prior * mse_loss(zA, references)
                if config.normal_prior == 3:
                    auloss1 = reference_prior
            ...
            if config.ldm or config.diffcls:
                auloss = jnp.where(
                    batch["marker"], auloss, jnp.zeros_like(auloss))
                auloss = jnp.sum(auloss) / count
                if config.ae_only:
                    totalloss = auloss
                else:
                    zeta = zeta_fn(state.step)
                    totalloss = zeta*totalloss + (1-zeta)*config.eta * auloss
            metrics = OrderedDict(
    ...
    def step_valid(state, batch):
        ...
        if config.diffcls:
            zB = contexts
            if config.train_cls:
                cls_params = ema_bparams
            elif config.checker:
                cls_params = variables_clsA["params"]
            else:
                cls_params = variables_clsB["params"]
            z0 = f_normalize(batch["references"]) if config.checker else None
            (
    ...
    def step_sample(state, batch, config):
        ...
        if config.diffcls:
            ...
            model_bd = score_func.bind(params_dict, rngs=rngs_dict)
            d_rng, i_rng = jax.random.split(state.rng)
            fn = lambda *args, **kwargs: model_bd.score(
                *args, **kwargs, training=False)
            zB = context
            begin = time.time()
